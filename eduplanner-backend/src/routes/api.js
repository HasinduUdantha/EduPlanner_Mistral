...
import { generatePlan, updatePlan } from "../services/ollamaService.js";
import { generatePlanSchema, updatePlanSchema } from "../utils/validation.js";
...
router.patch("/plan/:planId/progress", async (req, res) => {
  ...
  try {
    const { planId } = req.params;
    const { progress, overallProgressPercentage } = req.body;

    const updated = await Plan.findByIdAndUpdate(
      planId,
      { progress, overallProgressPercentage, updatedAt: new Date() },
      { new: true }
    );
    ...
    console.log("Plan progress updated successfully:", updated);
    res.json(updated);
  } catch (err) {
