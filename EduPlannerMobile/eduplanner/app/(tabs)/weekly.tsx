...
import { getLatestStudyPlan, updateStudyPlanProgress, updateStudyPlan } from '../../utils/api';
...
export default function WeeklyPlanScreen() {
  ...
  // FIXED: Safe parameter extraction with error handling
  let params: any = {};
  try {
...
// In your weekly screen (weekly.tsx)
// In your weekly screen (weekly.tsx)
const loadPlan = async () => {
  try {
    ...
    console.log('🔍 Loading plan with params:', params);
    
    // Strategy 1: PRIORITIZE plan from navigation parameters
    if (params && typeof params.plan === 'string') {
      try {
        ...
        // Set the plan immediately and return - don't fall back to API
        setPlanData(planToLoad);
        
        if (planToLoad?.progress) {
          setProgress(planToLoad.progress);
        }
        
        console.log('✅ Successfully loaded specific plan from navigation');
        return; // Exit early - don't load latest plan
        
      } catch (e) {
        console.warn('⚠️ Failed to parse plan from params:', e);
        // Only continue to API if parsing fails
      }
    }
    
    // Strategy 2: Try to load by plan ID if provided (but no plan data)
    ...
    // Strategy 3: Load latest plan ONLY if no specific plan was requested
    ...
    console.log('✅ Plan successfully loaded:', planToLoad._id);
  ...
  const handleFeedbackSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Your study plan has been updated based on your feedback!');
    } catch (error) {
  ...
  const getDayProgress = (day: number) => {
    ...
    const totalItems = (dayData.topics?.length || 0) + (dayData.activities?.length || 0);
    if (totalItems === 0) return 0;
    
    const completedItems = Object.values(progress[dayKey] || {}).filter(Boolean).length;
    ...
    return (
      <View style={styles.container}>
        <Card style={styles.errorCard}>
          <Card.Content>
            <MaterialIcons name="error" size={64} color={theme.colors.error} style={styles.errorIcon} />
    ...
    return (
      <View style={styles.container}>
        <Card style={styles.emptyCard}>
          <Card.Content>
            <MaterialIcons name="school" size={64} color={theme.colors.textSecondary} style={styles.emptyIcon} />
  ...
  const { plan } = planData;
  const overallProgress = plan.days?.length > 0 
    ? plan.days.reduce((acc, day) => acc + getDayProgress(day.day), 0) / plan.days.length 
    : 0;
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.detailsCard}>
          <Card.Content>
            <View style={styles.detailsRow}>
              <Chip 
                icon="trending-up" 
                style={[styles.detailChip, { backgroundColor: `${theme.colors.primary}15` }]}
              ...
              <Chip 
                icon="calendar-clock" 
                style={[styles.detailChip, { backgroundColor: `${theme.colors.accent}15` }]}
              ...
              <Chip 
                icon="timer" 
                style={[styles.detailChip, { backgroundColor: `${theme.colors.warning}15` }]}
                textStyle={{ color: theme.colors.warning }}
              >
                {plan.daily_time?.split('/')[0]?.replace(/\*\*/g, '').trim() || 'Daily study'}
              </Chip>
            </View>
          </Card.Content>
        </Card>
        ...
        {plan.days?.map((day, dayIndex) => {
          ...
          return (
            <Card key={day.day} style={[styles.dayCard, isCompleted && styles.completedDayCard]}>
              <TouchableOpacity
                    ...
                    <View style={styles.dayTitleContainer}>
                      ...
                      <View style={styles.dayInfo}>
                        ...
                        <Text style={styles.dayMeta}>
                          {day.time_required} minutes • {(day.topics?.length || 0) + (day.activities?.length || 0)} items
                        </Text>
                      </View>
                    </View>
                  ...
                  <Card.Content style={styles.dayContentPadding}>
                    ...
                    {day.topics?.map((topic, topicIndex) => (
                      <Surface key={topicIndex} style={styles.topicContainer}>
                        <View style={styles.topicHeader}>
                          <Checkbox
                            status={progress[`day_${day.day}`]?.[`topic_${topicIndex}`] ? 'checked' : 'unchecked'}
                            onPress={() => toggleTask(`day_${day.day}`, `topic_${topicIndex}`)}
                        ...
                        {topic.sub_topics?.map((subTopic, subIndex) => (
                          <View key={subIndex} style={styles.subTopicItem}>
                            <Checkbox
                              status={progress[`day_${day.day}`]?.[`subtopic_${topicIndex}_${subIndex}`] ? 'checked' : 'unchecked'}
                              onPress={() => toggleTask(`day_${day.day}`, `subtopic_${topicIndex}_${subIndex}`)}
                    ...
                    {day.activities?.map((activity, activityIndex) => (
                      <View key={activityIndex} style={styles.activityItem}>
                        <Checkbox
                          status={progress[`day_${day.day}`]?.[`activity_${activityIndex}`] ? 'checked' : 'unchecked'}
                          onPress={() => toggleTask(`day_${day.day}`, `activity_${activityIndex}`)}
                          color={theme.colors.warning}
                        />
                        <MaterialIcons name="assignment" size={16} color={theme.colors.warning} style={styles.activityIcon} />
      ...
      <Portal>
        ...
        >
          <View style={styles.modalHeader}>
            <MaterialIcons name="feedback" size={32} color={theme.colors.primary} />
