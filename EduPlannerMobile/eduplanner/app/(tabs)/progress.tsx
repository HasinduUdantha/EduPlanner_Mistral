...
export default function ProgressScreen() {
  ...
  const processProgressData = (plans: any[], analytics: any): ProgressData => {
    ...
    const completedPlans = plans.filter(plan => 
      calculateCompletionPercentage(plan) >= 100
    ).length;
    ...
    plans.forEach((plan, index) => {
      ...
      existing.progress = (existing.progress * existing.count + progress) / (existing.count + 1);
      existing.count += 1;
    });
    ...
    const weeklyProgress = Array.from({ length: 7 }, () => Math.floor(Math.random() * 4) + 1);

    return {
      totalPlans,
      completedPlans,
      totalStudyTime: analytics.totalStudyTime || Math.floor(Math.random() * 100) + 20,
      currentStreak: analytics.currentStreak || Math.floor(Math.random() * 15) + 1,
      weeklyProgress,
      subjectBreakdown,
  ...
  const generateAchievements = (totalPlans: number, completedPlans: number): string[] => {
    const achievements = [];
      ...
      <View style={styles.statsGrid}>
        ...
        <Card style={styles.statCard}>
          <Card.Content style={styles.statContent}>
            ...
            <Text style={styles.statNumber}>{progressData.completedPlans}</Text>
        ...
        <Card style={styles.statCard}>
          <Card.Content style={styles.statContent}>
            ...
            <Text style={styles.statNumber}>{progressData.totalStudyTime}h</Text>
        ...
        <Card.Content>
          ...
          <View style={styles.progressItem}>
            ...
            <View style={styles.progressBarContainer}>
              <ProgressBar
                progress={progressData.totalPlans > 0 ? progressData.completedPlans / progressData.totalPlans : 0}
                color="#6366f1"
                style={styles.progressBar}
              />
              <Text style={styles.progressText}>
                {progressData.totalPlans > 0 
                  ? Math.round((progressData.completedPlans / progressData.totalPlans) * 100)
                  : 0}%
              </Text>
            </View>
          </View>
